...
drone.set_pitch(power) #往前移動的值設為power，也就是20
drone.move(duration) #往前移動duration秒，也就是1秒
drone.set_pitch(side) #往前移動的值設為side，也就是0
power = power / 2 #power變成power/2，也就是10
drone.set_roll(power) #往右移動的值設為power，也就是10
drone.move(duration) #往前移動duration秒，也就是1秒
drone.set_roll(side) #往右移動的值設為side，也就是0
power = power + 10 #power變成power+10，也就是20
drone.set_pitch(-power) #往後移動的值設為power，也就是20
drone.move(duration) #往後移動duration秒，也就是1秒
drone.set_pitch(side) #往後移動的值設為side，也就是0
power = 30 - power #power變成30-power，也就是10
drone.set_roll(-power) #往左移動的值設為power，也就是10
